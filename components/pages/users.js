...
function Users() {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const response = await axios.get('https://api.lokivpn.com/api/users/dashboard-user');
        const usersWithSubscriptions = await Promise.all(
          response.data.map(async user => {
            try {
              const subscriptionResponse = await axios.get(`https://api.lokivpn.com/api/users/dashboard-user${user.UserID}/subscription-details`);
